...
import { deleteTask } from '@/lib/opportunities/task-actions';

type Props = {
  taskId: string;
  opportunityId: string;
  /** Rótulo curto (ex: "T001"/"T001.1") — usado no atributo `title` do botão. */
  label: string;
  /** Título completo da tarefa, interpolado na confirmação. */
  taskTitle: string;
  /** Quantidade de subtarefas DIRETAS — 0 numa linha de subtarefa (D-01, nunca tem filhas). */
  childCount: number;
};

/**
 * Botão 🗑️ de exclusão de tarefa/subtarefa com confirmação em cascata
 * (TASK-06). Clona a estrutura de `DeleteRiskButton.tsx` (overlay `z-[60]`,
 * confirm/cancel, `useTransition`, `router.refresh()` após sucesso) — única
 * diferença de conteúdo: quando `childCount > 0` a confirmação nomeia a
 * quantidade de subtarefas que serão removidas junto (zero-one-many:
 * singular para 1, plural para N). A remoção em si é responsabilidade do
 * banco (`on delete cascade`, 0037) — esta confirmação é a mitigação de
 * T-16-12 (exclusão acidental em cascata).
 */
export function DeleteTaskButton({
  ...
  function confirm() {
    ...
    startTransition(async () => {
      const result = await deleteTask(taskId, opportunityId);
      if (!result.ok) {
          ...
          <div className="bg-wh rounded-2xl shadow-2xl max-w-md w-full overflow-hidden">
            ...
            <div className="px-5 py-4">
              <p className="text-[12px] text-txt mb-3">
                Você está prestes a excluir a tarefa <strong>{taskTitle}</strong>
                {childCount > 0 && (
                  <>
                    {' '}
                    e {childCount === 1 ? 'sua' : 'suas'}{' '}
                    <strong>{childCount}</strong>{' '}
                    {childCount === 1 ? 'subtarefa' : 'subtarefas'}
                  </>
                )}
                .
              </p>
